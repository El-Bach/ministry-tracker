// src/screens/TeamMemberFieldsScreen.tsx
// Manage custom team member field definitions — add, edit, reorder, toggle active
...
export default function TeamMemberFieldsScreen() {
 ...
 const load = async () => {
  const { data } = await supabase
   .from('team_member_field_definitions')
   .select('*')
   .order('sort_order');
 ...
 const handleAdd = async () => {
  ...
  const { error } = await supabase.from('team_member_field_definitions').insert({
 ...
 const handleDelete = (field: FieldDefinition) => {
  Alert.alert(
   'Delete Field',
   `Delete "${field.label}"? All existing team member data for this field will be lost.`,
   [
    { text: 'Cancel', style: 'cancel' },
    {
     ...
     style: 'destructive',
     onPress: async () => {
      await supabase.from('team_member_field_definitions').delete().eq('id', field.id);
      load();
     },
    },
   ]
 ...
 const moveField = async (index: number, dir: -1 | 1) => {
  ...
  await Promise.all(
   updated.map((f) =>
    supabase.from('team_member_field_definitions').update({ sort_order: f.sort_order }).eq('id', f.id)
   )
  );
 };
