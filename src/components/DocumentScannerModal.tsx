...
export default function DocumentScannerModal({
  ...
  const loadRequirements = useCallback(async (): Promise<ReqItem[]> => {
    try {
      ...
      return reqs.map((r: any) => ({ id: r.id, title: r.title, stopName: nameMap[r.stop_id] ?? 'Stage' }));
    } catch { return []; }
  }, [taskId]);
  ...
  function enterCropMode() {
    ...
    Image.getSize(
      ...
      (nw, nh) => {
        ...
        // Initialize crop rect to full image display area
  ...
  async function applyCrop() {
    ...
    try {
      ...
      // Map display coords → native image pixel coords
  ...
  function makeCornerPanResponder(corner: 0 | 1 | 2 | 3) {
    ...
    return PanResponder.create({
      ...
      onPanResponderMove: (_, gs) => {
        ...
        const MIN_SIZE = 50;
        if (corner === 0) { // TL: adjust x, y, w (inverse dx), h (inverse dy)
        ...
        } else if (corner === 1) { // TR: adjust y, w
          r.y = Math.max(0, Math.min(r.y + dy, r.y + r.h - MIN_SIZE));
          r.w = Math.max(MIN_SIZE, Math.min(r.w + dx, imgDisplayW - r.x));
          r.h = Math.max(MIN_SIZE, r.h - dy);
        } else if (corner === 2) { // BL: adjust x, h
          r.x = Math.max(0, Math.min(r.x + dx, r.x + r.w - MIN_SIZE));
          r.w = Math.max(MIN_SIZE, r.w - dx);
  ...
  // Preview image dimensions (also used by crop helpers above)
  // containerW and containerH are computed in the crop helpers section
          ...
          >
            ...
            {!cropMode ? (
              ...
              <View style={s.cropActionsRow}>
                <TouchableOpacity style={[s.cropActionBtn, { borderColor: theme.color.danger }]} onPress={() => setCropMode(false)} disabled={applying}>
                  <Text style={[s.cropActionBtnText, { color: theme.color.danger }]}>✕ Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[s.cropActionBtn, { borderColor: theme.color.success, backgroundColor: theme.color.success + '22' }]} onPress={applyCrop} disabled={applying}>
                  {applying ? <ActivityIndicator size="small" color={theme.color.success} /> : <Text style={[s.cropActionBtnText, { color: theme.color.success }]}>✓ Apply Crop</Text>}
                </TouchableOpacity>
              </View>
            )}
