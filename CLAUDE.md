...
> Last updated: session 36 (Plan limit enforcement: 3-day grace period warning modal + full app lock after expiry; PlanEnforcementWrapper in App.tsx; PLAN_LIMITS moved to config.ts; migration_plan_enforcement.sql)
...
| Session | Changes |
|---|---|
| 36 | **Plan limit enforcement — grace period + app lock**: New `supabase/migration_plan_enforcement.sql` adds `plan_limit_exceeded_at TIMESTAMPTZ` column to `organizations`. `src/lib/config.ts` gains `PLAN_LIMITS` (moved from AccountScreen) + `GRACE_PERIOD_DAYS = 3`. `src/types/index.ts` `Organization` interface updated: plan union expanded to `'free' \| 'basic' \| 'premium' \| 'starter' \| 'business'`; `plan_limit_exceeded_at` field added. New `src/lib/planEnforcement.ts` helper `checkPlanLimits()` — counts active files + members, writes/clears `plan_limit_exceeded_at` in DB, returns `{ exceeded, isLocked, daysRemaining, ... }`. New `src/components/PlanWarningModal.tsx` — dismissible bottom-sheet warning shown every launch during 3-day grace period; shows days remaining countdown + "Upgrade Now" (WhatsApp) / "Remind Me Later"; non-owners see "contact your owner". New `src/components/PlanLockedScreen.tsx` — non-dismissible full-screen lock replaces entire app after grace period expires; owner sees "Upgrade Now" + "Contact Support"; non-owners see "contact your owner". `App.tsx` gains `PlanEnforcementWrapper` inner component (inside AuthProvider) that runs `checkPlanLimits` on every launch after auth resolves and renders the appropriate state. `AccountScreen.tsx` now imports `PLAN_LIMITS` from `config.ts` (no local redefinition). |
