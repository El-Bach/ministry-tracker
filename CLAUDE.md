...
> Last updated: session 34 (Service document sub-requirements; Required Docs modal in NewTaskScreen + TaskDetailScreen + CreateScreen; header layout redesign in TaskDetailScreen; auto due-date on file close; duplicate file copies assignee + stages; dashboard search clear on back; multi-select service filter fix; RESULT C/V USD on archived cards; financial label renames)
