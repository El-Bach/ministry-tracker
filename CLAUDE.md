...
> Last updated: session 54 (Ministry Contacts feature — per-stage picker on TaskDetail with inline display under stage name; archive trigger now ONLY the FINAL stage; "Closed" → "Received & Closed" rename; archived files show final stage status everywhere; due_date displays as ARCHIVED date when archived; restore from archive resets final stage to Pending and clears dates; section dividers between TaskDetail sections; financials P&L overhauled — column headers + uniform 13px; Contract+Balance card 14px; compact one-line filter rows on dashboard; Help Guide + FAQ updated; pen-edit on stage name removed)
